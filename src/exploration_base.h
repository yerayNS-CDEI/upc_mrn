...
namespace upc_mrn
{
    ...
    protected:
        ...
        bool isValidGoal(const geometry_msgs::msg::Pose &goal, double &path_length);
        double computePlanLength(std::vector<geometry_msgs::msg::PoseStamped> poses) const;
                                                                 ...
                                                                 got_map_(false)
    {
        ...
        // action of nav2
        nav2_client_ = rclcpp_action::create_client<NavToPose>(
            this,
    ...
    bool ExplorationBase::isValidGoal(const geometry_msgs::msg::Pose &goal, double &path_length)
    {
        ...
        //         RCLCPP_DEBUG(this->get_logger(), "Goal Valid! Path total distance: %f m", path_length);
        //         return true;
        //     }
        // }
        // else
            RCLCPP_ERROR(this->get_logger(), "ExplorationBase::isValidGoal(): Failed to call service get_plan");

        return false;
    }

    double ExplorationBase::computePlanLength(std::vector<geometry_msgs::msg::PoseStamped> poses) const
    {
        double length = 0.0;
        if (poses.size() >= 1)
        {
